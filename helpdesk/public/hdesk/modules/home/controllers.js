...
mainApp.controller('TicketController',
    ['$scope', '$rootScope','$location', '$routeParams', 'HDTicketService', 'HDUtilService','HDUserService',
        function ($scope,$rootScope, $location, $routeParams, HDTicketService, HDUtilService,HDUserService) {
            ...
            $scope.isStatusEnable=function(id){
                return (!((resData.status.statusId+1)==id) || ((resData.status.statusId-1)==id));
            }
            ...
            $scope.changeStatus = function (statusId2,userComment,updateFlag) {
                if(statusId2==3 && (!updateFlag)){
                    $scope.verify(3);
                    return;
                }
                ...
                var mode =1;
                if($scope.resData.status.statusId>statusId2){
                    mode=-1;
                }
                ...
                //ticketId,fromStatusId,mode,comment
                $rootScope.resetMessagePoint();
                HDTicketService.changeStatus($scope.resData._id,$scope.resData.status.statusId,mode,comment).success(function (response) {
                    $rootScope.formSuccessMessagePoint('200','Ticket status changed successfully.');
                    $scope.resData.status=response.status;
                    console.log('ticket status changed..');
                }).error(function (errResponse) {
                    $rootScope.formErrorMessagePoint(404, errResponse.errorMessage);

                    console.error('error in changing status tkt: ' + errResponse);
                });
            };
            ...
            HDTicketService.getSortedStatus().success(function (response) {
...
//Ticket controller
mainApp.controller('TicketFlowController',
    ['$scope', '$rootScope', '$routeParams', 'HDTicketService',
        function ($scope,$rootScope,  $routeParams, HDTicketService) {
            ...
            if ($routeParams.ticketId) {
                $scope.myparam = $routeParams.ticketId;
                HDTicketService.getTicketFlow($routeParams.ticketId).success(function (resData) {
...
console.log('ticket flow');
                    ...
                    console.log('ticket flow failed..' + resData);
                });
            }
            else {
                $scope.myparam = 'not present';
...
mainApp.controller('ChangeStatusCtrl', function ($scope,$rootScope, $uibModal, $log,HDUserService,HDTicketService) {
    $scope.changeStatus2 = function (statusId2,userComment) {
        if(statusId2==3){
            $scope.verify(3);
            return;
        }
        ...
        var mode =1;
        if($scope.$parent.resData.status.statusId>statusId2){
            mode=-1;
        }
        var comment='default comment..'
        if(userComment){
        ...
        //ticketId,fromStatusId,mode,comment
        HDTicketService.changeStatus($scope.$parent.resData._id,$scope.$parent.resData.status.statusId,mode,comment).success(function (response) {
            $rootScope.formSuccessMessagePoint('200','Ticket status changed successfully.');
            $scope.$parent.resData.status=response.status;
            console.log('ticket status changed..');
        }).error(function (errResponse) {
            $rootScope.formErrorMessagePoint(404, errResponse.errorMessage);
    ...
    console.log('RES DATA::: '+$scope.$parent.resData.subject);

    $scope.verify = function (statusId) {
        var modalInstance = $uibModal.open({
            animation: true,
            templateUrl: 'changeStatusModel.html',
            ...
            resolve: {
                ...
                changeStatusMethod: function () {
                    return  $scope.$parent.changeStatus;
                }
            }
        });
...
mainApp.controller('ChangeStatusModalInstanceCtrl', function ($scope, $uibModalInstance,$rootScope,HDTicketService,ticket,statusId,changeStatusMethod) {
    $scope.ticket=ticket;
    $scope.statusId=statusId;

    $scope.yes = function () {
        $rootScope.resetMessagePoint();
        changeStatusMethod(3,'Hello world..',true);
        $uibModalInstance.close('hellox');
        //$rootScope.formSuccessMessagePoint('200','test status');
    };
